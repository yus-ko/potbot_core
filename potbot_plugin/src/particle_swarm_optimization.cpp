...
PLUGINLIB_EXPORT_CLASS(
  potbot_nav::path_planner::ParticleSwarmOptimization,
  potbot_base::PathPlanner)
...
void ParticleSwarmOptimization::initialize(std::string name, tf2_ros::Buffer * tf)
{
  ...
  dsrv_ =
    new dynamic_reconfigure::Server<potbot_plugin::ParticleSwarmOptimizationConfig>(private_nh);
  dynamic_reconfigure::Server<potbot_plugin::ParticleSwarmOptimizationConfig>::CallbackType cb =
    boost::bind(&ParticleSwarmOptimization::reconfigureCB, this, _1, _2);
  dsrv_->setCallback(cb);
}

void ParticleSwarmOptimization::reconfigureCB(
  const potbot_plugin::ParticleSwarmOptimizationConfig & param, uint32_t level)
{
...
void ParticleSwarmOptimization::pointvec_to_pcl(
  const std::vector<potbot_lib::Point> & points_input,
...
void ParticleSwarmOptimization::point_to_marker(
  const potbot_lib::Point & points_input,
...
void ParticleSwarmOptimization::point_to_marker(
  const Eigen::VectorXd & points_input,
  visualization_msgs::Marker & marker_output)
{
  point_to_marker(
    potbot_lib::Point(
      points_input(0), points_input(1), points_input(
        2)), marker_output);
}

void ParticleSwarmOptimization::mat_to_marker_points(
  const Eigen::MatrixXd & points_input,
  visualization_msgs::Marker & marker_output)
{
  double g_resolution = 0.1;
...
void ParticleSwarmOptimization::path_to_line_marker(
  const std::vector<potbot_lib::Pose> & vector_input,
  visualization_msgs::Marker & marker_output)
{
