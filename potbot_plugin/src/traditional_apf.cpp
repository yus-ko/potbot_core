...
{
using potbot_lib::utility::get_distance;
using potbot_lib::utility::get_vector;
using potbot_lib::utility::get_pose;
using potbot_lib::utility::get_tf;
using potbot_lib::utility::get_point;
void TraditionalAPF::initialize(std::string name, tf2_ros::Buffer * tf)
{
  tf_ = tf;
  ros::NodeHandle private_nh("~/" + name);

  pub_potential_vector_ = private_nh.advertise<visualization_msgs::MarkerArray>(
    "potential_vector",
    1);
  ...
  dynamic_reconfigure::Server<potbot_plugin::TraditionalAPFConfig>::CallbackType cb = boost::bind(
    &TraditionalAPF::reconfigureCB, this, _1, _2);
  dsrv_->setCallback(cb);
}

void TraditionalAPF::reconfigureCB(
  const potbot_plugin::TraditionalAPFConfig & param,
  uint32_t level)
{
...
void TraditionalAPF::calculateCommand(geometry_msgs::Twist & cmd_vel)
{
  ...
  double omega = atan2(potential_.potential[1], potential_.potential[0]) - tf2::getYaw(
    robot_pose_.pose.pose.orientation);
...
void TraditionalAPF::calculatePotential()
{
  ...
  for (size_t i = 0; i < scan_.ranges.size(); i++) {
    ...
    double th = static_cast<double>(i) * scan_.angle_increment + scan_.angle_min;
