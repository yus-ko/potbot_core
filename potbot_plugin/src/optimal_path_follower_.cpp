...
void OptimalPathFollower::initialize(std::string name, tf2_ros::Buffer * tf)
{
  ...
  pub_objective_function_ =
    private_nh.advertise<sensor_msgs::PointCloud2>("objective_function", 1);

  dsrv_ = new dynamic_reconfigure::Server<potbot_plugin::OptimalPathFollowerConfig>(private_nh);
  dynamic_reconfigure::Server<potbot_plugin::OptimalPathFollowerConfig>::CallbackType cb =
    boost::bind(&OptimalPathFollower::reconfigureCB, this, _1, _2);
  dsrv_->setCallback(cb);
}

void OptimalPathFollower::reconfigureCB(
  const potbot_plugin::OptimalPathFollowerConfig & param,
  uint32_t level)
{
  optimizer_.setMargin(param.stop_margin_angle, param.stop_margin_distance);
  optimizer_.setLimit(
    param.min_linear_velocity, param.max_linear_velocity,
    param.min_angular_velocity, param.max_angular_velocity);
...
bool OptimalPathFollower::reachedTarget()
{
  ...
  // return path_index_ == target_path_.size()-1 &&
  //   abs(getDistance(target_point_)) <= stop_margin_distance_;
