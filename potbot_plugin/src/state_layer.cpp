      ...
      if (iter != ukf_id_.end()) {
        if (state_estimator_ == KALMAN_FILTER) {
          ...
          std::tuple<Eigen::VectorXd, Eigen::MatrixXd,
            Eigen::MatrixXd> ans = states_kf_[index_ukf].update(od, dt);
          ...
          std::tuple<Eigen::VectorXd, Eigen::MatrixXd,
            Eigen::MatrixXd> ans = states_kf_[index_ukf].update(observed_data, dt);
          Eigen::VectorXd xhat = std::get<0>(ans);
          Eigen::MatrixXd P = std::get<1>(ans);
          Eigen::MatrixXd K = std::get<2>(ans);
          ...
          // ROS_INFO(
          //   "estimated: id:%d, th:%3f,%3f, dt:%f",
          //   id, theta, tf2::getYaw(obstacle.pose.orientation), dt);
          ...
          observed_data << x, y;
          std::tuple<Eigen::VectorXd, Eigen::MatrixXd,
            Eigen::MatrixXd> ans = states_ukf_[index_ukf].update(observed_data, dt);
          Eigen::VectorXd xhat = std::get<0>(ans);
          Eigen::MatrixXd P = std::get<1>(ans);
          Eigen::MatrixXd K = std::get<2>(ans);
...
void StateLayer::onInitialize()
{
  ...
  while (ss >> source) {
    ...
    if (data_type == "LaserScan") {
      ...
      sync_.reset(
        new message_filters::Synchronizer<MySyncPolicy>(
          MySyncPolicy(10), sub_rgb_,
          sub_depth_, sub_info_));
  ...
  pub_obstacles_scan_estimate_ = nh.advertise<potbot_msgs::ObstacleArray>(
    "obstacle/scan/estimate",
    1);
  pub_scan_range_ = nh.advertise<geometry_msgs::PolygonStamped>("scan_range", 1);
  pub_scan_clustering_ =
    nh.advertise<visualization_msgs::MarkerArray>("obstacle/scan/clustering", 1);
...
void StateLayer::laserScanCallback(const sensor_msgs::LaserScanConstPtr & message)
{
  ...
  // クラスタリング結果をpotbot_msgs::ObstacleArray型に変換して取得
  scanclus.toObstaclearray(clusters_obstaclearray_scan);
  // for (auto& obs : clusters_obstaclearray_scan.data)
  //   obs.header = clusters_obstaclearray_scan.header;

  // 1時刻前のクラスタからの追跡
  static potbot_msgs::ObstacleArray clusters_obstaclearray_pre;
  potbot_lib::utility::associate_obstacle(
    clusters_obstaclearray_scan, clusters_obstaclearray_pre,
    *tf_);

  scanclus.setClusters(clusters_obstaclearray_scan);

  // クラスタをワールド座標系に変換して1時刻先の追跡用データにする
  potbot_lib::utility::get_tf(
    *tf_, clusters_obstaclearray_scan, global_frame_,
    clusters_obstaclearray_pre);

  visualization_msgs::MarkerArray clusters_markerarray;
  // クラスタリング結果をvisualization_msgs::MarkerArray型に変換して取得
  scanclus.toMarkerarray(clusters_markerarray);
  for (auto & clus : clusters_markerarray.markers) {
    clus.header = scan_data.header;
  }
  ...
  const sensor_msgs::CameraInfo::ConstPtr & info_msg)
{
  ...
  cv::Mat cameraMatrix = cv::Mat(
    3, 3, CV_64F, reinterpret_cast<void *>(
      const_cast<double *>(info_msg->K.data()))).clone();

  // 回転行列 R (3x3行列)
  cv::Mat R = cv::Mat(
    3, 3, CV_64F, reinterpret_cast<void *>(
      const_cast<double *>(info_msg->R.data()))).clone();

  // 投影行列 P (3x4行列)
  cv::Mat projectionMatrix = cv::Mat(
    3, 4, CV_64F, reinterpret_cast<void *>(
      const_cast<double *>(info_msg->P.data()))).clone();
  ...
  for (const auto & rect : detections) {
    ...
    if (debug_) {
      ...
      mean_point = mean_point / static_cast<double>(obstacle.points.size());
...
void StateLayer::applyCloud(const potbot_msgs::ObstacleArray & obstacles)
{
  ...
  if (!obstacles.data.empty() && dt > 0) {
    ...
    for (const auto & obs : obstacles.data) {
      ...
      ROS_DEBUG(
        "estimated: id:%d, x:%f, y:%f, th:%f, vx:%f, vy:%f, w:%f, dt:%f, size:%f/%f",
        obs.id, wobs.pose.position.x, wobs.pose.position.y, yaw, vx, vy, omega, dt, size,
        apply_cluster_to_localmap_);
      if (size < apply_cluster_to_localmap_) {
        if (abs(omega) < max_estimated_angular_velocity_ &&
          abs(vx) < max_estimated_linear_velocity_ && abs(vy) < max_estimated_linear_velocity_)
        {
